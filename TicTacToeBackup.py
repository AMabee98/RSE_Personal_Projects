...
score_grid, totalScore, tempCords
gameEnd, redCirBool, greenCirBool, blueCirBool, redSquareBool
...
try:
    ...
    def startGame(): 
        pickUpFromStack()
        n.move_pose(0.160, -0.061, 0.079, 0, 1.57, 0) #AI will start in bottom right corner everytime. 
    ...
    #When a peice is placed by the human or machine, update that spot in the array.
    ...
    #Have the machine pick up a peice off of the designated stack.
    def pickUpFromStack():
    ...
    #Randomly place a peice in any open spot.  
    def placePeiceLogic():
    ...
    #After each turn, check to see if there is three in a row.
    def checkGameEnd():
    ...
    #When the game is over, move all player and machine parts to their respective stack.
    def cleanUp():
        ...
        while num <= 8:
            if score_grid[num] == -1:
                n.move_pose(0.16, 0, 0.35, 0, 1.57, 0) #The location of the machine stack.
            num = num + 1
    ...
    while gameEnd == False:
        ...
        placePeiceLogic() #thinks about what peice to play next and uses placePeice() and Update from AI
        checkGameEnd() #Will check for a tie game or a winner 

    cleanUp() #When the game is over clean up the board.
